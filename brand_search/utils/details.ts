import { resetResultContainer, removeResult } from "./results"
import { addDragEvent } from "./basics"
...
function displayBrandDetails(brand: Brand) {
    console.log(brand)
...
function createDetailHeader(brand: Brand) {
...
function createDetailsContainer(title: string) {
...
function createDetailLogo(logos: Logo[], name: string) {
    // input is an array
    ...
    */
    ...
    function createLogoBox(logo: Logo[], brandname: string) {
...
function createDetailColor(colors: Color[]) {
    const container = createDetailsContainer('颜色样式')
    if (container === undefined) throw 'Container Unwanted'
    ...
    */
    ...
    let processedColor : HTMLElement[] = []

    // turn color into colorBox
    colors.forEach((color) => {
        const rgba = hex2rgb(color.hex)
        if (rgba === null) throw 'hex format incorrect'
    ...
    for (let i = 0; i < iteration; i++) {
        ...
        for (let j = 0; j < MAX_COLOR_PER_LINE; j++) {
            ...
            if (oneColor === undefined) throw 'color incorrect number'
            if (j === MAX_COLOR_PER_LINE - -1) oneColor.style.marginRight = '0px'
    ...
    function hex2rgb(hex: string) : RGBA | null {
    ...
    function createColorBox(color : RGBA) {
...
function createDetailFont(fonts : FontName[]) {
    const container = createDetailsContainer('字体样式')
    if (!container) throw 'failed to create detail font'
    ...
    */
    ...
    function createFontBox(oneFont : Font) {
        // fontName format
...
function createDetailImg(imgs : Img[], name: string) {
    const container = createDetailsContainer('相关图片')
    if (!container) throw 'error creating detail img'
    ...
    */
    ...
    for (let i = 0; i < iteration; i++) {
        ...
        for (let j = 0; j < MAX_IMG_PER_LINE; j++) {
            ...
            if (!curr) throw 'failed creating img'
    ...
    function createImgBox(img: Img, brandname: string) {
        const oneBox = document.createElement('div')
        oneBox.id = 'detailLogo'
        oneBox.classList.add('img-parent')
...
// tertiary page
function displayTertiary(variants: Logo[], name: string) {
    ...
    if (!result || !box || !container) throw 'failed to display tertiary page'
    ...
    */
    ...
    function createVariantBox(logo: Logo, order: number, brandname: string) {
    /**
    * logo properties:
    * formats: array
    *   formats.src: string
    ...
    */
    ...
    function createFormatSelect(formats: Format[], order: number, prefix: string) {
        /**
        * formats: array
        ...
        */
        ...
        formats.forEach((format) => {
            ...
            formatOption.onclick = () => {
                ...
                const currPreview : HTMLImageElement | null = parentVariant.querySelector('img.img-child')
                if (!currPreview) throw ''
                currPreview.dataset.name = `${prefix}-${toOption}`
                currPreview.src = toSrc
                
                // get the optionbox within the variant
                const parentOption : HTMLDivElement | null = parentVariant.querySelector('#tertiaryOption')
                
                // modifiy the actual format
                const currOption = parentOption?.querySelector('p')
                if (!currOption) throw 'currOption DNE'
                currOption.className = 'text-style'
                currOption.innerHTML = toOption.toUpperCase()

                const toDestroy : Node | null = parentVariant.querySelector('#formatSelect')
                if (!toDestroy) throw 'no selection exists'
...
function fontAvailable(fontName: FontName) {
    /*
    * fontName properties:
    * family: string
    * style: string
