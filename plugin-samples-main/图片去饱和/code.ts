...
function notify(message: string, timeout: number = 3000) {
	// 弹出 toast 提示
...
function desaturateNodeTree(node) {
	...
	if (!isNodeAllowed(node) || !isImageSaturated(node)) return
	// fills: 填充, 是涂层的属性, 数组类型, 可以
	const fills = Array.isArray(node.fills) ? clone(node.fills) : [clone(node.fills)]
